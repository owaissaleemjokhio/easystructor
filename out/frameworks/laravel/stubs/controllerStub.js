...
class ${name}Controller extends Controller
{
    ...
    public function __construct(private ${name}Service $${(0, stringUtils_1.toCamel)(name)}Service) {}
    ...
    {
        $records = $this->${(0, stringUtils_1.toCamel)(name)}Service->all();
       ...
       if (is_string(\$records)) {
        ...
        return \$this->response(
            new PaginatedCollection($records, '${(0, stringUtils_1.toSnake)(name)}', ${name}Resource::class),
            200,
    ...
    {
        \$record = \$this->${(0, stringUtils_1.toCamel)(name)}Service->store(\$request);
        ...
        return \$this->response(
            ["${(0, stringUtils_1.toSnake)(name)}" => new ${name}Resource(\$record)],
            201,
    ...
    {
        \$record = \$this->${(0, stringUtils_1.toCamel)(name)}Service->find(\$id);
    ...
    {
        \$record = \$this->${(0, stringUtils_1.toCamel)(name)}Service->update(\$id, \$request);
    ...
    {
        \$deleted = \$this->${(0, stringUtils_1.toCamel)(name)}Service->delete(\$id);
